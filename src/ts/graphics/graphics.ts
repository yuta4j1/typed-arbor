...
const graphics = {
  ...
  bacground(r: number, g: number, b: number, a: number): void {
  ...
  Oval(x: number, y: number, w: number, h: number, style: OvalStyle) {
    return new _Oval(x, y, w, h, style);
  },
  oval(x: number, y: number, w: number, h: number, style: OvalStyle): void {
    _Oval.prototype._draw(x, y, w, h, style);
  },
  line(x1: number, y1: number, x2: number, y2: number, style: PathStyle): void {
    const p = new _Path(x1, y1, x2, y2);
    p.draw(style);
  },
  lines(x1: number, y1: number, x2: number, y2: number): void {
